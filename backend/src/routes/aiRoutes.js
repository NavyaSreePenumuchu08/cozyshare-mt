...
router.post("/suggest-task", async (req, res) => {
  ...
  priority = "High";
  reason = "Dirty dishes can affect kitchen hygiene and should be completed quickly.";
} else if (
  text.includes("trash") ||
  ...
  suggestedLocation = "Kitchen";
  priority = "Medium";
  reason = "Cleaning kitchen surfaces helps maintain hygiene and prevents food-related mess.";
} else if (
  ...
  priority = "Medium";
  reason = "Laundry is important for cleanliness but is usually not urgent unless clothes are piled up.";
} else if (
  text.includes("fold") ||
  ...
  priority = "Low";
  reason = "Organizing clothes improves room cleanliness but is usually less urgent.";
} else if (
  text.includes("floor") ||
  text.includes("dust") ||
  text.includes("dusty") ||
  ...
  priority = "Medium";
  reason = "Cleaning the floor improves comfort, hygiene, and shared living cleanliness.";
} else if (
  ...
  priority = "High";
  reason = "Bathroom cleaning is important for hygiene and should not be delayed.";
} else if (
  text.includes("bed") ||
  ...
  suggestedLocation = "Living room";
  priority = "Medium";
  reason = "The living room is a shared space, so keeping it tidy improves household comfort.";
} else if (
  ...
  priority = "Medium";
  reason = "Plants and garden areas need regular care to stay healthy and clean.";
} else if (
  ...
  suggestedLocation = "Kitchen";
  priority = "Low";
  reason = "Grocery updates help household planning but are usually not immediately urgent.";
} else if (
  ...
  priority = "Medium";
  reason = "Missing essential items should be handled soon to avoid household inconvenience.";
} else if (
  text.includes("bill") ||
  ...
  suggestedLocation = "General";
  priority = "High";
  reason = "Bills with due dates are important and delays can cause penalties or service issues.";
} else if (
  ...
  suggestedLocation = "General";
  priority = "High";
  reason = "Maintenance issues can affect safety, comfort, or daily household use.";
} else if (
  ...
  suggestedLocation = "Living room";
  priority = "Low";
  reason = "Window and curtain cleaning improves freshness but is usually not urgent.";
} else if (
  ...
  suggestedLocation = "General";
  priority = "High";
  reason = "Pet care tasks are time-sensitive and important for animal hygiene and wellbeing.";
} else if (
  text.includes("guest") ||
  ...
  suggestedLocation = "Living room";
  priority = "Medium";
  reason = "Preparing shared spaces before guests arrive improves comfort and presentation.";
} else {
  taskTitle = "General household task";
  suggestedLocation = "General";
  priority = "Medium";
  reason = "This task helps keep the shared household organized and manageable.";
}
    res.json({
  suggestedTask: {
    title: taskTitle,
...
router.post('/suggest-assignee', async (req, res) => {
  try {
    const { householdCode, members, taskPriority } = req.body
  ...
  const assignmentDate = chore.createdAt || chore.updatedAt || chore.dueDate
  ...
  if (assignmentDate) {
    ...
    if (!currentLastAssignment || newAssignmentDate > currentLastAssignment) {
  ...
  const scoreDifference =
    workloadA.workloadScore - workloadB.workloadScore
  ...
  const totalDifference =
    workloadA.totalChores - workloadB.totalChores
...
const assignmentReason =
  `${suggestedAssignee} has the lowest workload score ` +
  `(${selectedWorkload.workloadScore} points), ` +
  `${selectedWorkload.pendingChores} pending chore(s), and was last assigned ` +
  `${lastAssignedText}. The new ${taskPriority || 'Medium'} priority task adds ` +
  `${incomingTaskWeight} workload point(s).`
